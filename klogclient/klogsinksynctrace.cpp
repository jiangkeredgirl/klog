...
KlogSinkSyncTrace& KlogSinkSyncTrace::instance()
{
	static KlogSinkSyncTrace _instance;
	return _instance;
}

int KlogSinkSyncTrace::Connect(const string& ip, int port, bool async)
{
	int error_code = 1;
	m_server_ip = ip;
	m_server_port = port;
...
int KlogSinkSyncTrace::Disconnect()
{
	if (m_tcp_client)
	{
...
int KlogSinkSyncTrace::OnTcpDisconnect(int status)
{
...
int KlogSinkSyncTrace::OnTcpRead(const char* data, size_t size, int status)
{
	if (data)
	{
...
int KlogSinkSyncTrace::OnTcpWrite(const char* data, size_t size, int status)
{
	if (data)
	{
...
int KlogSinkSyncTrace::SendKlogClientType()
{
	if (m_serial_parse)
	{
...
int KlogSinkSyncTrace::SendEvent(const string& serial_event_data)
{
