...
void MenuBarui::setupUi(QMenuBar* hostWidget)
{
	...
	if (m_hostWidget)
	{
		...
		m_viewMenu->addAction(tr("函数调用栈"));
		m_viewMenu->addAction(tr("函数调用流"));
		m_viewMenu->addAction(tr("函数思维图"));
		m_viewMenu->addAction(tr("函数执行时间靶点图"));
		m_viewMenu->addAction(tr("清空log"));
		m_viewMenu->addAction(tr("重置默认配置"));
