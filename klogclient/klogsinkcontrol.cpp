...
KlogSinkControl::KlogSinkControl()
{
}
...
KlogSinkControl& KlogSinkControl::instance()
{
	static KlogSinkControl _instance;
	return _instance;
}

int KlogSinkControl::Connect(const string& ip, int port, bool async)
{
	int error_code = 1;
	m_server_ip = ip;
	m_server_port = port;
...
int KlogSinkControl::Disconnect()
{
	if (m_tcp_client)
	{
...
int KlogSinkControl::OnTcpConnect(int status)
{
	if (status == 0)
	{
...
int KlogSinkControl::OnTcpDisconnect(int status)
{
...
int KlogSinkControl::OnTcpRead(const char* data, size_t size, int status)
{
	if (data)
	{
...
int KlogSinkControl::OnTcpWrite(const char* data, size_t size, int status)
{
	if (data)
	{
...
int KlogSinkControl::SendKlogClientType()
{
	if (m_serial_parse)
	{
...
int KlogSinkControl::GetKlogServerPort()
{
	if (m_serial_parse)
	{
...
int KlogSinkControl::SendEvent(const string& serial_event_data)
{
