...
void TestKlog()
{
	...
	KlogWarn("警告日志");
	KlogError("错误日志");
	KlogError("klog测试对即时函数的调用不是常量表达式: 假设一个错误码:%d", 1);
	KlogError("klog测试对即时函数的调用不是常量表达式: 假设一个错误码:{}", 1);
	//CreateConsole;
	//GenerateDump;
	//KlogCreateStdout;
	//KlogSetStdoutUTF8;
...
void TestSpdlog()
{
	...
	SLOG_WARN("警告日志");
	SLOG_ERROR("错误日志");
	SLOG_INFO("错误	C7595	“std::basic_format_string<char,int>::basic_format_string”: 对即时函数的调用不是常量表达式");
	spdlog::error("spdlog测试对即时函数的调用不是常量表达式: 假设一个错误码:%d", 1);
	spdlog::error("spdlog测试对即时函数的调用不是常量表达式: 假设一个错误码:{}", 1);
	SLOG_ERROR("spdlog测试对即时函数的调用不是常量表达式: 假设一个错误码:%d", 1);
	SLOG_ERROR("spdlog测试对即时函数的调用不是常量表达式: 假设一个错误码:{}", 1);
}
